...
public class CaomTapQuery {
    ...
    public String profilerCheckpointID;
    ...
    private VOTableDocument execQuery(String uri, String adql)
        ...
        final Profiler profiler = new Profiler(CaomTapQuery.class);
        if (CredUtil.checkCredentials()) {
            queryAuthMethod = AuthMethod.CERT;
        }

        if (StringUtil.hasText(profilerCheckpointID)) {
            profiler.checkpoint(profilerCheckpointID + ".postCheckCredentials");
        }
        ...
        if (httpDownload.getThrowable() != null) {
            throw new TransientFault("query failed: " + uri, httpDownload.getResponseCode(), httpDownload
                .getThrowable());
        }
